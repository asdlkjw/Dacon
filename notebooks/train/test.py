...
def do_voc_evaluation(gts_df, preds_df):
    ...
    result = eval_detection_voc(
        pred_boxlists=pred_boxlists, gt_boxlists=gt_boxlists, iou_thresh=0.85
    )
    ...
    for gt_boxlist, pred_boxlist in zip(gt_boxlists, pred_boxlists):
        ...
        pred_bbox = np.array(
            [
                ...
                for _, rbox in pred_boxlist.iterrows()
            ]
        )
        ...
        gt_bbox = np.array(
            [
                ...
                for _, rbox in gt_boxlist.iterrows()
            ]
        )
        ...
        for l in np.unique(np.concatenate((pred_label, gt_label)).astype(int)):
            ...
            iou = rboxlist_iou(pred_bbox_l, gt_bbox_l)
...
def main():
    ...
    parser.add_argument(
        "-answer", "--answer", dest="answer", default="/data/submit2 copy.csv"
    )
    parser.add_argument("-data", "--data", dest="data", default="/data/submit2.csv")
